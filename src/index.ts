...
const createWindow = (): void => {
  ...
  // M5StickC Plus2 advertises BLE as DEVICE_NAME "M5-Controller" (see
  // M5-BlueSignal firmware). Electron has no native chooser UI for Web
  // Bluetooth, so auto-pick that device (or the first one found) when the
  // renderer calls navigator.bluetooth.requestDevice(). The event can fire
  // multiple times as the scan discovers more devices -- only resolve once
  // the M5 shows up, or give up after a timeout (don't cancel on the first,
  // likely-empty tick).
  let bluetoothScanTimeout: NodeJS.Timeout | null = null;
  window.webContents.on(
    "select-bluetooth-device",
    (event, deviceList, callback) => {
      event.preventDefault();
      console.log(
        "BLE scan tick, devices seen:",
        deviceList.map((d) => d.deviceName)
      );
      const device = deviceList.find((d) => d.deviceName === "M5-Controller");
      if (device) {
        if (bluetoothScanTimeout) {
          clearTimeout(bluetoothScanTimeout);
          bluetoothScanTimeout = null;
        }
